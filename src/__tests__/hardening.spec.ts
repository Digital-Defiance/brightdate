...
describe("hardening", () => {
  ...
  describe("fromDate validation", () => {
    ...
    it("accepts a cross-realm-like Date object (toString tag duck-type)", () => {
      ...
      const vm = require("vm");
      const crossRealmDate: Date = vm.runInNewContext("new Date(946727935816)");
  ...
  describe("fromUnixMs validation", () => {
    ...
    it("accepts 0 (Unix epoch)", () => {
      // TAI-UTC offset at 1970 = 10 (pre-1972 sentinel)
      // bd = (0 + 10 − 946727967.816) / 86400 ≈ -10957.499511759259
      expect(fromUnixMs(0)).toBeCloseTo(-10957.499511759259, 6);
    });
  });
  ...
  describe("fromUnixSeconds validation", () => {
    ...
    it("accepts 0", () => {
      expect(fromUnixSeconds(0)).toBeCloseTo(-10957.499511759259, 6);
    });
  });
