...
describe('calendar', () => {
  ...
  describe('fromCalendar', () => {
    it('creates a BrightDate of exactly 0 for J2000.0 (2000-01-01T12:00:00Z)', () => {
      expect(fromCalendar(2000, 1, 1, 12, 0, 0)).toBe(0);
    });
  ...
  describe('NOTABLE_EPOCHS', () => {
    ...
    it('UNIX epoch is exactly -10957.5', () => {
      expect(NOTABLE_EPOCHS.UNIX).toBe(-10957.5);
    });

    it('Y2K is exactly -0.5 (midnight before J2000.0 noon)', () => {
      expect(NOTABLE_EPOCHS.Y2K).toBe(-0.5);
    });

    it('GPS epoch is exactly -7300.5', () => {
      // GPS epoch is 1980-01-06 00:00:00 UTC, exactly 7300.5 days before J2000 noon
      expect(NOTABLE_EPOCHS.GPS).toBe(-7300.5);
    });
