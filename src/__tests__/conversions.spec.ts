...
describe("conversions", () => {
  ...
  describe("fromDate / toDate", () => {
    ...
    it("Unix epoch (1970-01-01T00:00:00Z) ≈ -10957.4995", () => {
      const unixEpoch = new Date(0);
      const bd = fromDate(unixEpoch);
      // 1970-01-01T00:00:00Z: TAI-UTC offset = 10 (pre-1972 sentinel)
      // bd = (0/1000 + 10 − 946727967.816) / 86400 ≈ -10957.499511759259
      expect(bd).toBeCloseTo(-10957.499511759259, 6);
    });

    it("Y2K midnight (2000-01-01T00:00:00Z) ≈ -0.499257", () => {
      const y2k = new Date(Date.UTC(2000, 0, 1, 0, 0, 0));
      // TAI-UTC = 32 at Y2K midnight
      // bd = (946684800 + 32 − 946727967.816) / 86400 ≈ -0.499257129629453
      expect(fromDate(y2k)).toBeCloseTo(-0.499257129629453, 8);
    });
  ...
  describe("fromISO / toISO", () => {
    ...
    it("2000-01-01T11:58:55.816Z → 0 (J2000.0 UTC label)", () => {
      expect(fromISO("2000-01-01T11:58:55.816Z")).toBe(0);
    });

    it("2000-01-01T12:00:00.000Z → TT noon offset", () => {
      // TT noon is 64.184 s after J2000 UTC label → ≈ 0.000742870 days
  ...
  describe("utcToTaiBrightDate / taiToUtcBrightDate", () => {
    // In v1.0 BrightDate uses a TAI substrate; the UTC/TAI distinction is
    // handled internally. These functions are now deprecated identity
    // wrappers retained for back-compat.
    it("utcToTaiBrightDate is identity (deprecated)", () => {
      const bd = fromISO("2020-01-01T00:00:00Z");
      expect(utcToTaiBrightDate(bd)).toBe(bd);
    });

    it("taiToUtcBrightDate is identity (deprecated)", () => {
      const bd = fromISO("2023-06-15T12:00:00Z");
      expect(taiToUtcBrightDate(bd)).toBe(bd);
    });

    it("round-trip is exact (both are identity)", () => {
      const bd = fromISO("2020-06-15T12:00:00Z");
      expect(taiToUtcBrightDate(utcToTaiBrightDate(bd))).toBe(bd);
    });
  });
  ...
  describe("normalize", () => {
    ...
    it("converts the J2000.0 UTC-label ISO string to 0", () => {
      expect(normalize("2000-01-01T11:58:55.816Z")).toBe(0);
    });
